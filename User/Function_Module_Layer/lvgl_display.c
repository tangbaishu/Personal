...
void LCD_Display_Test(void)
{
	...
	lcd_init();                             /* ԵʼۯLCD */
    g_point_color = RED;
    sprintf((char *)lcd_id, "LCD ID:%04X", lcddev.id);  /* ݫLCD IDղӡսlcd_id˽ة */
    ...
    while (1)
    {
        ...
        lcd_show_string(10, 130, 240, 16, 16, (char *)lcd_id, RED); /* ДʾLCD ID */
        x++;
...
void lvgl_bare_test(void)
{   
    extern HAL_StatusTypeDef HAL_TickInit(uint32_t TickPriority);
    HAL_TickInit(TICK_INT_PRIORITY);
	lv_init();
	lv_port_disp_init();
	lv_port_indev_init();
...
void lvgl_RTOS_task(void *param)
{
	lv_init();
    lv_port_disp_init();
    lv_port_indev_init();
